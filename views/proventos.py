...
import views.proventos_futuro as pf
...
def render_proventos_view():
    ...
    # --- Bloco de Consulta Investidor10 (Sempre visível no topo) ---
    col_bt1, col_bt2 = st.columns([2, 1])
    with col_bt2:
        pf.render_proventos_futuros_header()

    pf.render_proventos_futuros_results()
            ...
            if ano > ano_mais_antigo:
                ...
                for i, mes_nome in enumerate(meses_ordem):
                    if i < mes_limite:
                        ...
                        html_table.append(f'      <td style="padding: 14px 16px; background-color: var(--table-header-bg); border-bottom: 1px solid var(--border-color); text-align: right; color: #00CC96;">{val_fmt}</td>')
                    else:
