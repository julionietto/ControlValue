...
*   **API do Google Gemini:** Utilizada como o motor de inteligência artificial generativa para:
    ...
    *   **Análise do portfólio do investidor, inferência de perfil e objetivo, e geração de narrativas macroeconômicas e recomendações estratégicas para o Relatório Executivo (no `executive_report_service.py`).**
...
*   **Google Gemini API (GenAI):** Plataforma de inteligência artificial generativa, utilizada para:
    ...
    *   **Inferência do perfil de investidor, análise de cenário macroeconômico e geração de orientações estratégicas para o Relatório Executivo.**
...
*   **`pandas`:** Biblioteca amplamente utilizada para manipulação e análise de dados, essencial em módulos como `db/options.py`, `utils/formatters.py` e **agora no `executive_report_service.py` para tratamento eficiente de tabelas, dados financeiros e preparação para gráficos/relatórios.**
...
*   **`ReportLab`:** Uma biblioteca Python para criar documentos PDF de forma programática. Utilizada no `executive_report_service.py` para gerar relatórios executivos profissionais, permitindo controle granular sobre layout, estilos, tabelas e parágrafos.
...
*   **Modularidade e Extensibilidade:** A arquitetura baseada em orquestrador e agentes externos permite adicionar novos passos ou modificar existentes com relativa facilidade, sem impactar o fluxo principal. **A criação de um novo serviço (`executive_report_service.py`) e uma nova view (`views/report_executivo.py`) para o Relatório Executivo demonstra a capacidade de estender a aplicação com novas funcionalidades complexas de forma modular, sem alterar o core do sistema.**
...
*   **Caminho do Arquivo:** A funcionalidade de geração de relatório executivo foi centralizada no arquivo `services/executive_report_service.py`. Este arquivo foi **movido do diretório raiz para o subdiretório `services` (`executive_report_service.py -> services/executive_report_service.py`)** para organizar melhor a arquitetura de serviços da aplicação, garantindo que a lógica de negócios complexa seja encapsulada e reutilizável.
...
*   **Serviço de Geração de Relatório (`services/executive_report_service.py`):**
    *   Este serviço é o coração da funcionalidade, responsável por buscar, processar e analisar os dados do portfólio.
    *   **Fluxo de Dados:**
