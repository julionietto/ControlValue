...
*   **Estratégia Otimizada de Busca e Cache de Cotações:** O processo de obtenção de cotações em tempo real foi significativamente aprimorado para aumentar a performance e a robustez:
    *   A função `_fetch_prices_batch` (em `services.py`), utilizada para buscar preços de múltiplos tickers, agora adota uma estratégia híbrida:
    ...
    *   **Melhorias na Resiliência da Busca de Cotações:** Essas funções de busca, juntamente com `get_usd_brl_rate`, `get_btc_usd_rate`, e `get_ibov` em `services.py`, beneficiam-se de uma lógica aprimorada de normalização de tickers (`_normalize_ticker_for_yf`) para suportar uma variedade ainda maior de formatos (ações da B3, ações americanas, índices como `^BVSP`, criptoativos), além de um tratamento mais resiliente para extração de dados de fechamento e o uso aprimorado de `fast_info` como fallback.
