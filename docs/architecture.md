...
O diagrama a seguir ilustra o fluxo principal do pipeline de deploy automatizado. É importante notar que funcionalidades sob demanda, como o "Report Executivo Inteligente", operam de forma interativa na interface do usuário e não estão representadas neste fluxo sequencial.
...
*   **Agente de Report Executivo (`services/executive_report_service.py` e `views/report_executivo.py`):** É um novo agente de inteligência de negócio que opera sob demanda, não fazendo parte do pipeline de deploy automatizado. Ele é responsável por:
    *   Coletar dados abrangentes do portfólio do usuário (ativos, proventos, etc.) diretamente do PostgreSQL.
    *   Inferir o perfil de risco e os objetivos do investidor com base na composição da carteira.
    *   Analisar o desempenho dos ativos para identificar aqueles sob pressão ou desconto.
    *   Interagir com a API do Google Gemini para gerar uma narrativa macroeconômica viva e personalizada, incluindo fatos relevantes e orientações estratégicas.
    *   Orquestrar a geração de um relatório profissional em formato PDF, pronto para download, utilizando a biblioteca `reportlab`. A interface para este agente é provida pelo módulo `views/report_executivo.py`.
...
*   **API do Google Gemini:** Utilizada como o motor de inteligência artificial generativa para:
    ...
    *   **Geração de análises macroeconômicas e estratégicas personalizadas para o 'Report Executivo' (no `services/executive_report_service.py`).**
...
*   **`reportlab`:** Biblioteca Python para a criação programática de documentos PDF, essencial para a geração de relatórios executivos profissionais.

## 5. Princípios de Design

*   **Automação Inteligente:** Redução da intervenção manual em tarefas de deploy através da automação e uso de IA para decisões estratégicas (versionamento, commit, documentação) e diagnóstico de problemas. As regras para esta automação, incluindo a execução obrigatória do pipeline, são formalmente definidas em `.agents/AGENTS.OD`. Agora se estende à **Inteligência Executiva**, fornecendo análises macroeconômicas dinâmicas, inferência de perfil de risco e orientação estratégica personalizada através do 'Report Executivo', tudo impulsionado por IA, transformando dados brutos em insights acionáveis.
...
**Estrutura de Diretórios Complementar:**

*   `services/`: Este é um novo diretório introduzido para encapsular a lógica de serviços de negócio complexos, promovendo a modularidade e a separação de responsabilidades. Atualmente, hospeda o `executive_report_service.py`, mas é projetado para expandir com outros serviços especializados da aplicação.
*   `docs/relatorio_investimentos_*.pdf`: Exemplos de relatórios executivos em PDF gerados pelo sistema, demonstrando a capacidade de personalização e geração de documentos profissionais.
...
### 7.9. Geração de Relatórios Executivos Inteligentes (`views/report_executivo.py` e `services/executive_report_service.py`)

Este novo componente oferece ao usuário a capacidade de gerar sob demanda um relatório executivo detalhado de sua carteira, impulsionado por inteligência artificial.

*   **`services/executive_report_service.py`:** É o coração da lógica de negócio deste relatório.
    *   `get_user_portfolio_data(user_id)`: Responsável por consultar o banco de dados PostgreSQL para obter todos os dados relevantes do portfólio de um usuário (ativos, transações, proventos).
    *   `infer_investor_profile_and_goal(active_df)`: Analisa a composição da carteira para inferir automaticamente o perfil de risco (Conservador, Moderado, Arrojado) e o objetivo principal do investidor (Renda Passiva, Crescimento, Misto).
    *   `analyze_asset_performance(active_df)`: Identifica ativos que estão sob pressão ou desconto, baseando-se em métricas de preço (preço médio vs. valor justo/teto) e fatores setoriais ou macroeconômicos pré-definidos.
    *   `generate_ai_macro_narrative(...)`: Integra-se com a API do Google Gemini para construir uma análise macroeconômica (Brasil e EUA) e estratégica personalizada, justificando o desempenho dos ativos e fornecendo orientações de rebalanceamento.
    *   `generate_executive_pdf_report(user_id)`: Orquestra todas as etapas anteriores e, usando a biblioteca `reportlab`, compila os dados e a análise da IA em um documento PDF formatado profissionalmente, incluindo KPIs, detalhamento de ativos e as narrativas inteligentes. Retorna os bytes do PDF para a interface.

*   **`views/report_executivo.py`:** Implementa a interface do usuário no Streamlit para esta funcionalidade.
    *   Fornece botões para "Emitir Report Executivo Atualizado" e "Baixar Report Executivo em PDF".
    *   Exibe KPIs sumarizados do portfólio (Patrimônio Investido, Perfil de Risco, Objetivo, Saúde do Portfólio).
    *   Apresenta a análise macroeconômica e estratégica gerada pela IA em abas dedicadas.
    *   Detalha os ativos sob pressão/desconto identificados.
    *   Mostra um `st.dataframe` com a posição atual dos ativos.
    *   Gerencia o estado da sessão (`st.session_state.report_data`) para cachear o relatório gerado, evitando reprocessamento desnecessário.
