...
*   **Serviço de Geração de Relatório (`services/executive_report_service.py`):**
    ...
    *   **Fluxo de Dados:**
        1.  **Coleta de Dados:** `get_user_portfolio_data(user_id)` se conecta ao banco de dados PostgreSQL (`psycopg2`) para buscar todos os ativos, proventos históricos e informações do usuário. Utiliza `pandas` para manipular os DataFrames. **Recentemente, a função foi expandida para incluir o cálculo de `total_compras_ano` (o total de aportes no ano corrente) no conjunto de dados retornados, fornecendo uma métrica crucial para a análise financeira anual.**
        ...
        5.  **Geração de PDF:** `generate_executive_pdf_report(user_id)` integra todos os dados coletados e as análises da IA para construir um documento PDF profissional usando a biblioteca `ReportLab`. O relatório inclui:
            ...
            *   **Uma tabela de KPIs expandida para 5 colunas, agora incluindo "Aportes (Ano)" ao lado de "Saldo Atual", "Total Investido", "Proventos Totais" e "Aderência Estratégica", com larguras de coluna ajustadas e padding otimizado.**
...
*   **Interface do Usuário (`views/report_executivo.py`):**
    ...
    *   **Apresenta 5 cards de KPIs resumidos, incluindo a nova métrica "Aportes (Ano)" (referente ao ano corrente), juntamente com "Saldo Atual (Mercado)", "Total Investido", "Proventos Totais Acumulados" e "Lucro/Prejuízo Real", fornecendo um panorama financeiro imediato e mais completo.**
