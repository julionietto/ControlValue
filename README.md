...
*   **Integração com Agente de Documentação:** Após os testes, um `doc_agent.py` é executado para criar ou atualizar automaticamente a documentação do projeto, mantendo-a sempre alinhada com as alterações de código.
...
*   `doc_agent.py`: Responsável pela criação e atualização automatizada da documentação (como `README.md` e `docs/architecture.md`), mantendo-a alinhada com as alterações de código.
