...
*   **`yfinance` (Yahoo Finance API Wrapper):** Biblioteca Python utilizada para buscar cotações de mercado em tempo real e dados históricos de ativos (ações, FIIs, ETFs, criptomoedas) de diversas bolsas. Implementa uma estratégia de busca híbrida e otimizada. A função `_normalize_ticker_for_yf` foi aprimorada para reconhecer e formatar corretamente uma ampla gama de tickers, incluindo ações da B3 (ex: `MGLU3`), FIIs (`MXRF11`), ETFs de criptomoedas (`HASH11`), ações e REITs americanos (ex: `AAPL`, `NHI`), e índices (ex: `^BVSP`), garantindo compatibilidade com os formatos do Yahoo Finance. Para a busca de cotações, prioriza `fast_info` para as cotações mais recentes e rápidas, com `yf.download` como fallback robusto para garantir a disponibilidade de dados históricos, especialmente para ativos com histórico mais longo ou quando `fast_info` falha. As funções de busca de câmbio (`get_usd_brl_rate`, `get_btc_usd_rate`) e índices (`get_ibov`) foram refatoradas para utilizar uma nova função interna de busca em lote, `_fetch_indicators_batch`, que centraliza a coleta desses indicadores. Esta função emprega `yf.download` de forma mais robusta para dados históricos e inclui um fallback resiliente com `fast_info` quando necessário, garantindo a obtenção eficiente e confiável de dados para o Dólar, Bitcoin e IBOVESPA. **Adicionalmente, as funções `fetch_dividend_yields` e `_fetch_dy_batch` foram introduzidas em `services.py` para buscar o *Dividend Yield* (DY) de ativos. A `_fetch_dy_batch` tenta obter o DY diretamente da `ticker_obj.info` (primeiro `trailingAnnualDividendYield`, depois `dividendYield`) e, como fallback, calcula o DY com base nos dividendos históricos do último ano e a cotação atual. Os resultados são cacheados (`@st.cache_data(ttl=3600)`) e retornados em percentual, proporcionando uma métrica de desempenho de proventos essencial para a análise de portfólio.**
...
*   **SQLite:** Um banco de dados leve, sem servidor e embarcável, utilizado para persistência de dados em arquivos, com a localização desses arquivos tipicamente dentro do diretório `db/` (ex: `db/*.db`).
...
*   **Estratégia Otimizada de Busca e Cache de Cotações:** O processo de obtenção de cotações em tempo real foi significativamente aprimorado para aumentar a performance e a robustez:
    *   **Busca Primária Paralela (`yf.Ticker().fast_info`):** Prioriza a busca paralela usando `yf.Ticker(t).fast_info.get('lastPrice')` via `concurrent.futures.ThreadPoolExecutor`. Essa abordagem é **altamente eficiente e precisa para obter as cotações mais recentes**, ideal para mercados voláteis. Os resultados são imediatamente armazenados no cache de preços da sessão (`st.session_state.price_cache`).
    *   **Busca Secundária (`yf.download`) como Fallback:** Caso a `fast_info` falhe para algum ticker (retornando valor `0.0` ou `NaN`), o sistema realiza uma segunda tentativa usando `yf.download`. Esta é uma busca em lote que se mostra robusta para capturar a última cotação de fechamento válida para ativos que podem apresentar inconsistências com `fast_info`.
    *   **Busca em Lote de Indicadores de Mercado (`_fetch_indicators_batch`):** Uma nova função interna, `@st.cache_data(ttl=300)` `_fetch_indicators_batch`, foi introduzida em `services.py` para centralizar a busca de indicadores chave como a cotação do Dólar (`BRL=X`), Bitcoin (`BTC-USD`) e IBOVESPA (`^BVSP`). Esta função prioriza `yf.download` para obter dados de um período recente (5 dias) e tem um robusto fallback para `yf.Ticker().fast_info` em caso de falha ou valores inválidos, garantindo que os indicadores sejam sempre atualizados e confiáveis. As funções `get_usd_brl_rate`, `get_btc_usd_rate` e `get_ibov` agora delegam a esta função para obter seus respectivos valores, com padrões resilientes (ex: 5.15 para USD/BRL, 79000.0 para BTC/USD e 187000.0 para IBOV) em caso de falha completa da API. A chamada em `views/geral.py` para `get_btc_usd_rate` agora também passa o parâmetro `is_first_load`, garantindo que a lógica de cache e de verificação de mercado aberto seja aplicada consistentemente a todos os indicadores.
    *   **Gerenciamento Inteligente do Cache de Preços (`st.session_state.price_cache`):**
        *   O cache de preços na `st.session_state` (`price_cache`) é agora proativamente gerenciado para garantir que contenha apenas cotações válidas (`> 0.0`). Em cada inicialização ou re-renderização da "Visão Geral", o cache é limpo de quaisquer entradas com preços zero ou inválidos, forçando uma nova busca para esses tickers.
        *   A lógica de `final_tickers_to_fetch` foi ajustada para incluir tickers já no cache, mas com preços inválidos (`<= 0.0`), assegurando que estes sejam refetched.
        *   A atualização do `price_cache` com `new_prices` é feita iterativamente, garantindo que somente cotações positivas sejam armazenadas, prevenindo a persistência de dados incorretos. Esta abordagem minimiza chamadas desnecessárias à API externa e otimiza a experiência do usuário com dados mais rápidos e confiáveis.
    *   **Melhorias na Resiliência da Busca de Cotações:** As funções de busca de cotações de ativos e indicadores de mercado em `services.py` (`_fetch_prices_batch` e a recém-introduzida `_fetch_indicators_batch`) beneficiam-se de uma lógica aprimorada de normalização de tickers (`_normalize_ticker_for_yf`) para suportar uma variedade ainda maior de formatos (ações da B3, ações americanas, índices como `^BVSP`, criptoativos), além de um tratamento mais resiliente para extração de dados de fechamento e o uso aprimorado de `fast_info` como fallback, assegurando a coleta de dados de mercado mais precisa e confiável em diferentes cenários. **Com a introdução de `fetch_dividend_yields` em `services.py`, a `views/geral.py` também agora busca e utiliza o *Dividend Yield* (DY) para os ativos da carteira, permitindo uma análise mais completa de rendimentos.**

*   **Simplificação da Preparação de Dados para Gráficos:** As funções `render_visao_geral_view` foram refatoradas para simplificar a manipulação de dados antes da plotagem. A coluna temporária `Ativo_display`, que antes era utilizada para formatar os tickers dos ativos (`fii_prov_df['Ativo'].apply(format_ticker_for_display)`) e então usada nos eixos dos gráficos `px.bar`, foi eliminada. Agora, a formatação é aplicada diretamente à coluna `Ativo` original do DataFrame, e esta coluna já formatada é utilizada nos gráficos. Esta mudança reduz a criação de colunas intermediárias, otimizando levemente o consumo de memória e a clareza do código na camada de apresentação, sem alterar o fluxo de dados fundamental ou a lógica de cálculo dos proventos e retornos. **Adicionalmente, para garantir a consistência e robustez na manipulação de dados de ativos, o DataFrame `unified_df` é agora inicializado com uma lista explícita de colunas pré-definidas (`cols = ['id', 'ticker', 'Tipo', 'Quantidade', 'Preço', 'Valor da operação', 'Cotação Atual', 'Valor atualizado', 'Orientação', 'Lucro / Prejuízo', 'Peso %', 'currency']`). Essa prática assegura que o DataFrame sempre possua a estrutura esperada, mesmo quando `all_rows` estiver vazio, prevenindo erros em operações subsequentes e melhorando a estabilidade da interface.**
...
*   **Serviço de Geração de Relatório (`services/executive_report_service.py`):**
    ...
    *   **Fluxo de Dados:**
        1.  **Coleta de Dados:** `get_user_portfolio_data(user_id)` se conecta ao banco de dados PostgreSQL (`psycopg2`) para buscar todos os ativos, proventos históricos e informações do usuário. Utiliza `pandas` para manipular os DataFrames. **Recentemente, a função foi expandida para incluir o cálculo de `total_aportes_ano` (os aportes líquidos do ano corrente, considerando o balanço entre compras e vendas de ativos) no conjunto de dados retornados, fornecendo uma métrica crucial para a análise financeira anual.**
        ...
        3.  **Análise de Desempenho de Ativos:** `analyze_asset_performance(active_df)` identifica ativos que estão com preço médio acima do valor justo ou preço teto (`fair_value`, `price_ceiling`), ou que estão sob pressão setorial específica (ex: FIIs de escritório por vacância, REITs por juros altos nos EUA, commodities por desaceleração chinesa). **Esta função foi aprimorada para lidar de forma mais robusta com preços teto nulos ou inválidos e, crucially, para incorporar o símbolo da moeda (`R$` ou `U$`) dinamicamente nas mensagens de diagnóstico e nos dados dos ativos com desempenho inferior, melhorando a clareza e a aplicabilidade da análise para portfólios multi-moeda.**
        ...
        5.  **Geração de PDF:** `generate_executive_pdf_report(user_id)` integra todos os dados coletados e as análises da IA para construir um documento PDF profissional usando a biblioteca `ReportLab`. O relatório inclui:
            ...
            *   **Uma tabela de KPIs expandida para 5 colunas, agora incluindo "Aportes Líquidos (Ano)" ao lado de "Saldo Atual", "Total Investido", "Proventos Totais" e "Aderência Estratégica", com larguras de coluna ajustadas e padding otimizado.**
            *   A narrativa da IA, formatada em tópicos claros.
            *   Uma tabela de alertas para ativos sob pressão/desconto, com estilização condicional (vermelho). **A exibição desses ativos no PDF foi aprimorada para usar o símbolo de moeda correto (R$ ou U$) para o preço médio e a cotação, garantindo a consistência com a análise de desempenho.**
...
*   **Interface do Usuário (`views/report_executivo.py`):**
    ...
    *   **Apresenta 5 cards de KPIs resumidos, incluindo a nova métrica "Aportes Líquidos (Ano)" (referente ao ano corrente, calculados como a soma das compras subtraídas das vendas), juntamente com "Saldo Atual (Mercado)", "Total Investido", "Proventos Totais Acumulados" e "Lucro/Prejuízo Real", fornecendo um panorama financeiro imediato e mais completo.**
    ...
    *   **Na aba "📊 Detalhamento dos Ativos em Carteira" (anteriormente "Evolução Histórica dos Proventos e Média Mensal Real"), foram adicionadas as seguintes melhorias:**
        *   Um resumo "Foto Atual" para o ano corrente (YTD), indicando o total de proventos recebidos e a média mensal até o mês atual.
        *   **A apresentação do "Gráfico de Evolução Anual" foi aprimorada:** Para o ano corrente, o gráfico agora exibe o "Total de Proventos" e a "Média Mensal" como valores acumulados até o mês atual (Year-to-Date - YTD), proporcionando uma representação mais precisa e atualizada da performance do ano em curso, enquanto anos anteriores mantêm a média anual completa.
        *   Um novo gráfico interativo "📅 Proventos Mês a Mês no Ano Atual (Recebidos & Provisionados)", que visualiza os proventos efetivamente recebidos e os proventos futuros (provisionados) para cada mês do ano corrente, acompanhado de uma linha para a média acumulada. Isso oferece uma visão preditiva e detalhada do fluxo de caixa de proventos.
    *   **Na aba "⚠️ Análise Setorial & Ativos sob Desconto", a exibição dos ativos sob pressão foi aprimorada. Os títulos dos expansores para cada ativo agora exibem o preço médio e a cotação com o símbolo de moeda dinâmico (R$ ou U$), tornando a informação mais precisa e fácil de interpretar em um contexto de portfólio multi-moeda.**
