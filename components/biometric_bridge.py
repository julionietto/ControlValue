...
def biometric_register_component(options_json):
    ...
    (function() {{
        ...
        // Tenta executar no contexto principal para evitar bloqueio de iframe
        const targetNavigator = window.top.navigator;

        targetNavigator.credentials.create({{ publicKey: options }})
...
def biometric_authenticate_component(options_json):
    ...
    (function() {{
        ...
        targetNavigator.credentials.get({{ publicKey: options }})
