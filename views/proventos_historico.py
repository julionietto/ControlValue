...
def render_proventos_historico_view():
    ...
    st.markdown("""
        ...
        th { font-weight: bold !important; font-size: 1.10rem !important; color: #a1a1aa !important; text-transform: uppercase; }
        th:first-child { text-align: center !important; }
        td { font-size: 1.0rem; vertical-align: middle !important; }
        </style>
    ...
    # 1. Linha TOTAL
    tm_row = {'MÊS': '<div style="text-align: center; font-size: 1.10rem; font-weight: bold;">TOTAL</div>'}
    for col in col_order:
        ...
        tm_row[col] = f'<div style="text-align: right; font-size: 1.0rem; {color}">{val_fmt}</div>'
    ...
    if ano > ano_mais_antigo:
        ...
        growth_row = {'MÊS': f'<div style="text-align: center; font-size: 1.0rem;">📈</div>'}
        for col in col_order:
            val_curr, val_prev = totais_row[col], tot_prev_full[col]
            if st.session_state.get('hide_values', False):
                growth_row[col] = '<div style="text-align: right; font-size: 1.0rem;">••••••</div>'
            elif val_prev > 0:
                pct = ((val_curr / val_prev) - 1) * 100
                color = "#00CC96" if pct >= 0 else "red"
                growth_row[col] = f'<div style="color: {color}; text-align: right; font-size: 1.0rem;">{pct:,.2f}%</div>'.replace('.', ',')
            else:
                growth_row[col] = '<div style="text-align: right; font-size: 1.0rem;">0,00%</div>'
        ...
        for i, m in enumerate(meses_ordem):
            if i < mes_limite:
                ...
                avg_ytd_row[m] = f'<div style="font-size: 1.0rem; text-align: right; color: #00CC96;">{format_provento(media_mes)}</div>'
            else: avg_ytd_row[m] = ''
