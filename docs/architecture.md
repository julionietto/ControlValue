...
*   **`yfinance` (Yahoo Finance API Wrapper):** Biblioteca Python utilizada para buscar cotações de mercado em tempo real e dados históricos de ativos (ações, FIIs, ETFs, criptomoedas) de diversas bolsas. Implementa uma estratégia de busca híbrida e otimizada, priorizando `fast_info` para cotações mais recentes e rápidas, com `yf.download` como fallback robusto para garantir a disponibilidade de dados, especialmente para ativos com histórico mais longo ou quando a `fast_info` falha.
